...
> **注（2026-09-26，决策 D15）：本段是 2026-09-19 的实测修正记录，其中两个数字按性质区分。**
>
> - `1280.6 GiB` 是**旧代码估算 / 历史值**：当时 `unit_checkpoint_bytes` 只按 `backbone` 前缀
>   判断，而这一点正是本条上报的协议问题。
> - `8.34 GiB` **未复现**：该量应为 per-run `9,437,184,000 B ≈ 8.7891 GiB`，或按两次 attempt
>   预留的跑前门禁 `18,874,368,000 B ≈ 17.5781 GiB`。原值疑为 **GB 数值误标为 GiB**
>   （8.34 GiB = 8.955 GB，恰落在 `epoch_checkpoint_delivery.md` 记录的「约 44 MB…约需
>   8–9 GB/候选/seed」区间内）。
> - `≈319 GiB` 是**基于历史实测单文件大小（265.7 KB/epoch）的回填参考**，不是正式口径。
>
> **该协议问题已于 2026-09-26 结清（D15）**：常量 `RESNET18_COMPONENT_BYTES = 45 MiB`
> **未变**，被修正的是从未进入签署制品的适配器路径推广；判据改为
> `(training_path, backbone, model_family)` 三元组。正式口径以**代码输出**为准——
> 全 Pilot 累计 `348,443,368,000 B（≈324.5 GiB）`、跑前门禁
> `18,874,368,000 B（≈17.58 GiB）`、峰值单元 `cifar10/nnpu/native_cnn`。
> **向队友报磁盘需求请用这组数**，不要再用 1280.6 GiB / 35.16 GiB，也不要用 319 GiB / 8.34 GiB。
