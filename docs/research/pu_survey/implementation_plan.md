...
| $`\pi_{population}`$（正例占全池比例）与 5 seed 的关系 | 协议 §3.1 原字面"每个 seed 各自完整数据池"容易理解为每个 seed 不同；但 5 个 seed 只改变切分与标记方式、不改变样本本身，$`\pi_{population}`$ 是所有 seed 相同的一个常数。已按此修订协议 §3.1 | 已完成 |
| DIY 接口 vs 中心注册表 | 两处要求并存：§2.4 第 8 条要求提供自由的 DIY 参数接口（用户自定义模型/参数），§5.3 要求实验超参数全部经中心注册表统一管理（保证公平与可复现）。已按分层关系补充协议 §2.4 第 8 条：DIY 接口是工具箱面向所有用户的能力；中心注册表管理本实验候选配置，实验榜单仅使用注册表登记的配置 | 已完成 |
