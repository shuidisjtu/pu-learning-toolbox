...
对应协议 §2.4.1-3 的当前 Toolbox 实现状态（调研结论）：
...
**结论**：应新增独立的 `ExperimentRunner`（或 `PUExperiment`）层来承载用户划分、PA/OA 双协议、
模型选择记录及最终测试；**不宜改变现有 `PUPipeline` 以内部交叉验证为核心的语义**。

**实现侧自动验证要求**（协议 §2.4.7 的落地）：自动化测试必须验证索引不重叠、PA 不读取
真实验证标签、测试集不进入训练或选择。
