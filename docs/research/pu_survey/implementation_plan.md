...
- 标签语义（`fit` 的 `y` 属 PU / 监督 PN / PNU）同属"接入时的接口约定"：当前无声明字段，
  PU 的 `{1,0}` 与监督的 `{0,1}` 数值同形、错配静默。**确定时机同为 P3 接入前置**，
  方案与参考文献 2 的对照见 [label_semantics_plan.md](../../dev/label_semantics_plan.md)。
