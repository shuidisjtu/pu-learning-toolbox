...
| $`\pi_{population}`$（正例占全池比例）与 5 seed 的关系 | 协议 §3.1 字面是"每个 seed 各自完整数据池"的正例占比；但 5 个 seed 只改变切分与标记方式、不改变样本本身，所以 $`\pi_{population}`$ 应当对所有 seed 相同（一个常数：二分后全池的正例占比）。待办：把协议 §3.1 措辞改为"对每个 seed 定义于同一完整二元化数据池"，消除"每个 seed 不同"的歧义 | 建议修订 |
| DIY 接口 vs 中心注册表 | 两处要求并存：§2.4 第 8 条要求提供自由的 DIY 参数接口（用户自定义模型/参数），§5.3 要求实验超参数全部经中心注册表统一管理（保证公平与可复现）。需标明分层关系：DIY 接口是工具箱面向所有用户的能力；中心注册表是本实验管理候选配置的机制，实验榜单只能使用注册表登记的配置。待办：在协议中补一句说明 | 待补 |
