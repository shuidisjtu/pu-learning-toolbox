...
| 日期 | 方法 | 状态 | 代码与文档 | 验证 |
|---|---|---|---|---|
| 2026-08-10 | 测试金字塔重构（unit → integration → e2e） | 新建 `tests/integration/`（迁移 test_pipeline/test_pipeline_deep/test_run，marker unit→integration）与 `tests/e2e/`（迁移 workflow_scripts 3 文件并修正 cwd 深度，marker unit→e2e）；注册 integration/e2e marker（pyproject + check_test_quality 同步）；新增 8 个真实子进程 E2E 旅程（quickstart demo→auto run→report、profile→recommend 链、demo→profile→recommend→run 全链、错误旅程退出码 1/stderr/无 traceback、确定性复现）；CI 分层：PR 快层 `-m "not slow and not e2e"`，nightly 顶层 `-m "slow or e2e"` | `pyproject.toml`；`scripts/check_test_quality.py`；`.github/workflows/{tests,nightly}.yml`；`docs/dev/project_structure.md` §3（目录树 + marker 权威行 + 金字塔 CI 映射） | `811 passed`；6 门禁全绿；e2e 层 25 测试 nightly 执行 |
| 2026-08-10 | 架构图 Mermaid 化（C4 思想落地） | 以 `architecture.md` 为权威消除图-文档漂移：§2 分层表补 Orchestration 层（`workflows`/`cli`）与 User Layer 成员；新增 §2.1 系统上下文图 + §2.2 模块组件图（Mermaid 代码块，改文本即更新，零新依赖，GitHub 原生渲染）；删除 matplotlib 手绘架构图脚本及 PNG/SVG 产物（原先确认无任何文档引用） | `docs/dev/architecture.md`；`docs/figures/` 整目录删除 | Mermaid 本地渲染验证（2 图）；doc_links/math_rendering/test_quality/project_metadata/skill_sync/format 6 门禁全绿；无代码改动 |
| 2026-08-09 | 架构腐朽修复第二批（14 项发现,PR #15） | 真相分裂单源化：PU 零一风险委托 metrics、sigmoid 提取 `utils/activations.py`、canonical_hash 5 份→`benchmarks/_common.py`、class_prior 校验 12 处→`check_scalar_in_range`、y_true 校验 6 处→`validate_true_binary_labels`、KLDCE/LDCE 推导→`_class_prior.py`、RBF 第 6 处→`rbf_weights`；测试与工程：`test_pnu_loss.py` 新增、benchmark marker 双份清理、check_test_quality 默认严格（含模块级 pytestmark 检测回归修复）、nightly CI（slow 套件）、.gitignore 白名单 18→2 行、`n_features_out` 别名删除；局部性：`fit_evaluate` 275→203 行拆 3 个私有 helper（行为零变化）；`docs/superpowers/` 本地化（gitignore 忽略 + 从远程移除） | 15 任务逐任务双审 + opus 全分支评审（2 项 truth-divergence 修复）；`docs/dev/architecture_audit.md` §5 第二批治理记录；`decision_log.md` 追加 | `802 passed`；6 门禁全绿；slow 套件 nightly 可执行 |
...
| 2026-08-08 | PUSB Table 2 strict 完整执行 | 45 shards 有界并行、失败重试、4500 key 精确聚合、逐单元配对 Student-t 95% CI | parallel/aggregate/report runner 与 `pusb_table2_strict_full/` | 4500/4500 trials；0 失败；CV/最终优化 4500/4500 收敛；accuracy 微平均 PUSB/uLSIF=`0.7963/0.7597`；保持 `paper_claim=false` |
...
| 2026-08-06 | cnn 门禁收紧（encoder 注入契约） | `architecture="cnn"` 除 TORCH backend 外要求构造签名声明 `encoder` 参数（签名检测，fail-fast），消除 self_pu/dgpu 配 cnn 时 backbone 静默忽略；文档补深度算法接入契约 | `pu_toolbox/workflows/pipeline.py`；`tests/integration/test_pipeline_deep.py`；`docs/user/{howto/cli,howto/pipeline,reference/api}.md` | `632 passed`；4 项质量门禁 + ruff（check+format）全通过 |
...
| 2026-08-06 | CI 稳定性修复 | B905：test_pipeline_deep.py 两处 `zip` 补 `strict=False`（本地 lint 此前只查 `pu_toolbox/` 漏 `tests/`）；encoder determinism 测试改构建前 seed + 随机输入 + eval 模式 + 显式容差（消除 BN 对常数 batch 的 subnormal 平台噪声） | `tests/integration/test_pipeline_deep.py`；`tests/unit/estimators/test_vision.py` | CI 13 job 全绿（3 平台 × 3 Python + quality + wheel）；本地 `630 passed` |
...
| 2026-08-06 | 三子系统综合审计修复（假/大/空/兼容性） | 推荐器/Pipeline/CLI 25 项修复：auto 模式激活 GPU 维度并重算 _is_deep（torch 播种+成本警告对 auto 选中深度方法生效）；删陈旧 `_BUILTINS_REGISTERED` 标志（clear_registry 后静默 0 候选）；ScoringConfig 校验分数表≤锚点（防 >100）；稀疏输入显式 warning；`**kwargs` 不再误判为必填；CNN 实例路径 encoder 注入；profile 双跑消除（build_diagnostic_report(profile=)）；序列化助手 4 份拷贝收敛到 utils/serialization.py；save 非法 format 抛 ValueError；demo 守卫对齐默认 5 折 CV；错误捕获去重；.npz 友好报错；list-priors 去半硬编码；零消费者字段（cv_scores 删除、n_computed 序列化） | `pu_toolbox/advisor/`、`pu_toolbox/workflows/`、`pu_toolbox/cli/`、`pu_toolbox/utils/serialization.py`（新）等；`docs/user/reference/api.md`、`docs/user/howto/pipeline.md`、`docs/project_management/cli_design.md` | `647 passed`；4 项质量门禁 + ruff（check+format）全通过；SAR 排名翻转/GPU 激活/auto+torch 播种等 7 个新回归测试 |
