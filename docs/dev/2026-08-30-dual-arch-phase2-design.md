...
## 8. 实施顺序（待 writing-plans 产出后附）

1. 契约测试先行（能力声明升级）→ 类属性更新；
2. encoder 参数 + fit 组合逻辑（TDD）；
3. 涟漪更新（cnn_candidates / capability declarations）；
4. CV fold 隔离参数化扩展 + pipeline 端到端；
5. gpu marker + GPU 测试（FunctionTest 执行）；
6. 文档收尾与全量验证。
