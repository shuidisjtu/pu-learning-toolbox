...
- [x] clean-room 实现论文式 PU 权重目标、风险修正和共享神经特征提取器
- [x] 通过行级 bootstrap 重估并传播两个域类先验、标记率和倾向区间
- [ ] 增加概念漂移、支持集断裂和无漂移退化的完整统计协议
- [x] 实现 `trPU`、`tePU`、fine-tune、MMD、两步加权与损失修正消融
- [x] 增加一个公开表格数据集的多随机种子 smoke、95% CI 与样本重叠审计
- [ ] 复跑论文原始七个设置、完整调参/早停和多 seed 协议后再声明数值复现
- [ ] 作者未公开源码；无法升级 `SourceStatus` 为 `official_exact`
