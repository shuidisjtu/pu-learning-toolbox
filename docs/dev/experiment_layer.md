...
| `protocols.py` | 策略 ABC：`Generator.generate(X, y_true, c, seed)` + `output_view` 声明（PU 生成器 `"pu"`，oracle 生成器 `"clean"`）；`Trainer.fit(estimator, X, y, *, class_prior, val_pu)`；`SelectionProtocol.select(trajectories, val_part, threshold_candidates)` |
...
- **PN oracle 接入（2026-09-11，详见 [pn_oracle_integration.md](../research/pu_survey/pn_oracle_integration.md)）**：
  ...
  **PU 视图 + `SupervisedTrainer` 的误配在 runner 内 fail-loud**——此前该组合会静默把 SCAR
  标记当作真实标签训练出错误的"上界"。深度（CNN）oracle 的 clean-val checkpoint 选择列为 Phase 2
