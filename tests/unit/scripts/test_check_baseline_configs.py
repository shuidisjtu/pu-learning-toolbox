...
def _v2_config() -> dict:
    """The real locked v2 config, deep-copied for mutation per test."""
    path = c.CONFIGS_DIR / "seven_methods_pu_baseline_v2.json"
...
@pytest.mark.unit
def test_basic_v2_config_pins_constructor_defaults_exactly():
    """The shipped locked config matches live constructor defaults."""
    config = _v2_config()
    assert c.check_config("seven_methods_pu_baseline_v2.json", config) == []
