...
def _locked_config() -> dict:
    """The real locked v6 config, deep-copied for mutation per test."""
    path = c.CONFIGS_DIR / "seven_methods_pu_baseline_v6.json"
...
def test_basic_locked_config_pins_constructor_defaults_exactly():
    """The shipped locked config (v6) matches live constructor defaults."""
    config = _locked_config()
    assert c.check_config("seven_methods_pu_baseline_v6.json", config) == []
